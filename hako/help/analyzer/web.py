...
def render_dashboard_player(
    ...
    """Renders formatted json records and highlight guidelines for the Player dashboard view."""
    rows, eligibility, borders = [], [], []

    for name in sorted_players:
        row_data = (
            df_base.loc[df_base["Player"].str.startswith(name)].iloc[0]
            if any(df_base["Player"].str.startswith(name))
            else None
        )
        ...
        if name in analyzer.new_players:
            d_name += " ★"
        ...
        if target != "ignore" and target < analyzer.base_exp:
            if name.lower() in analyzer.main_roster_names:
                ...
                if subs:
                    sub_hover = f"Subbed by {', '.join(subs)}"
            else:
                d_name += " ▲"
                orig = analyzer.sub_relations.get(name.lower(), [])
                if orig:
                    sub_hover = f"Subbing for {orig[0]}"
        ...
        if analyzer.use_teams:
            ...
            try:
                row["Elo"] = float(analyzer.elo_map.get(name.lower(), np.nan))
            except Exception:
                row["Elo"] = np.nan

        if row_data is not None:
            tot, cor = analyzer.s_part[name], analyzer.c_counts[name]
            for key_details in ["Overall", "Type 1", "Type 2", "Type 3", "Chant"]:
                analyzer.player_song_details[name][key_details].sort(key=lambda s: s[2:].strip().lower())

            row.update(
                {
                    "GR": {
                        "count": float(row_data["GR"] * 100),
                        "details": [f"{cor}/{tot}"] + analyzer.player_song_details[name]["Overall"],
                    },
                    ...
                    if pd.notnull(row_data["Mean Over-8"])
                    else np.nan,
                }
            )
            ...
            if analyzer.use_teams:
                row.update(
                    {"Lives Taken": int(row_data["Lives Taken"]), "Lives Saved": int(row_data["Lives Saved"])}
                )
                ...
                row[t_labels[tid]] = (
                    {
                        ...
                        "details": [f"{succ}/{seen}"] + analyzer.player_song_details[name][f"Type {tid}"],
                    }
                    if pd.notnull(row_data[t_labels[tid]])
                    else np.nan
                )
            ...
            if watched:
                ...
                off_details = [
                    s for s in analyzer.player_song_details[name]["Overall"] if s[2:] not in rig_song_lines
                ]
                ...
                row.update(
                    {
                        ...
                        "Rig Over-8": float(row_data["Rig Over-8"])
                        if pd.notnull(row_data["Rig Over-8"])
                        else np.nan,
                        "Over-8 Δ": float(row_data["Over-8 Δ"])
                        if pd.notnull(row_data["Over-8 Δ"])
                        else np.nan,
                        "Rig GR": {
                            "count": float(row_data["Rig GR"] * 100),
                            "details": [f"{succ_rig}/{tot_rig}"] + analyzer.player_song_details[name]["Rigs"],
                        }
                        if pd.notnull(row_data["Rig GR"])
                        else np.nan,
                        "Off GR": {
                            "count": float(row_data["Off GR"] * 100),
                            "details": [f"{succ_off}/{tot_off}"] + off_details,
                        }
                        if pd.notnull(row_data["Off GR"])
                        else np.nan,
            ...
            if h_diffs:
                ...
                row["Mean Difficulty Hit"] = np.nan
            ...
            row["Chant GR"] = (
                {
                    ...
                    "details": [f"{succ_chan}/{seen_chan}"] + analyzer.player_song_details[name]["Chant"],
                }
                if pd.notnull(row_data["Chant GR"])
                else np.nan
            )
    ...
    if "GR" in df_players.columns and "Eru" not in analyzer.tour_label:
        if analyzer.val_str == "default":
            if analyzer.tour_label == "Watched 2+8s":
                th_val = "25, 20, 15, 10, 5"
            elif analyzer.tour_label in ["Watched", "QuagWatched"]:
                th_val = "28, 18, 12, 6"
            elif analyzer.tour_label in ["Usual", "Quagsual"]:
                th_val = "28, 19, 8"
            elif watched:
                th_val = "28, 18, 12, 6"
            else:
            ...
            th_val = analyzer.val_str

        try:
            th = [float(x.strip()) for x in th_val.split(",")] if th_val else []
        except Exception:
            th = [28.0, 18.0, 12.0, 6.0]

        gv = df_players["GR"].map(lambda x: x["count"] if isinstance(x, dict) else x).tolist()
        for t in th:
            f_idx = -1
            for i, v in enumerate(gv):
                if pd.notnull(v) and v >= t:
                    f_idx = i
            if f_idx != -1 and f_idx < len(df_players) - 1:
                borders.append(int(f_idx))

    desc_cols = [
        "Elo", "GR", "GR Δ", "UF", "UF Δ", "Score", "1/8s", "2/8s", "Lives Taken", "Lives Saved",
        "OP GR", "OP Δ", "ED GR", "ED Δ", "IN GR", "IN Δ", "Rigs", "Rig Rate", "Solo Rigs",
        "Solo Rig Rate", "Over-8 Δ", "Rig GR", "Off GR", "Rig Δ", "Median Vintage Hit", "Chant GR"
    ]
    ...
    elo_ser = (
        df_players["Elo"].fillna(0.0)
        if "Elo" in df_players.columns
        else pd.Series(0.0, index=df_players.index)
    )
    gr_ser = df_players["GR"].map(lambda x: x["count"] if isinstance(x, dict) else x).fillna(0.0)
    rig_ser = (
        df_players["Rigs"].map(lambda x: x["count"] if isinstance(x, dict) else x).fillna(0.0)
        if "Rigs" in df_players.columns
        else pd.Series(0.0, index=df_players.index)
    )
    ...
    for col in df_players.columns:
        ...
        if col in desc_cols or col in asc_cols:
            if col in int_cols or col in rate_cols or col in ["Mean Difficulty Hit", "Median Vintage Hit", "Median Time"]:
            ...
            if not num.dropna().empty:
                if col == "Chant GR" and num.dropna().max() == 0:
                    continue
    ...
    for _, row in df_players.iterrows():
        ...
        for col in headers:
            val = row[col]
            if col in ["Player", "Team", "Tier"] or isinstance(val, dict):
                row_dict[col] = val
...
def render_dashboard_tour(analyzer, watched: bool, tour_song_details: dict, player_song_details: dict) -> list[dict]:
    """Renders formatted JSON records for overall tour statistics."""
...
def render_dashboard_team(analyzer, df_teams: pd.DataFrame, team_song_details: dict) -> tuple[list[dict], dict]:
    """Renders formatted JSON records and highlight rules for Team statistics."""
    team_rows, team_hl_rules = [], {}
    if not analyzer.use_teams:
        return team_rows, team_hl_rules
    ...
    for _, row_data in df_teams.iterrows():
        ...
        if h["total_matches"] > 0:
            ...
            if w_line:
                details_hover.append(w_line)
            if l_line:
                details_hover.append(l_line)
            if t_line:
                details_hover.append(t_line)
        ...
        if h["total_matches"] > 0:
            ...
            item_payload["_win_pct_sort"] = -1.0
    ...
    if team_rows:
        ...
        for col in df_teams_temp.columns:
            if col.startswith("_"):
                continue
            num = (
                df_teams_temp["_win_pct_sort"]
                if col == "Win Record"
                else df_teams_temp[col].map(lambda x: x["count"] if isinstance(x, dict) else x)
            )
    ...
    for row in team_rows:
        ...
        for k, v in row.items():
            if k.startswith("_"):
                continue
            if k in ["Total 1/8s", "Win Record", "Team Leader"]:
                f_dict[k] = v
...
def render_dashboard_tier(analyzer, rows1: list[dict], rows2: list[dict], player_song_details: dict) -> dict:
    """Renders formatted JSON records grouped by tier."""
    tier_data = {}
    ...
    for r1, r2 in zip(rows1, rows2):
        ...
        for p in players_tracked:
            ...
            for song_line in player_song_details[p]["Overall"]:
                if not song_line.startswith("✓"):
                    continue
                song = song_line[2:].strip().lower()
                if song in taken_suffix:
                    contribution_details.append(f"✓ {song_line[2:]} {taken_suffix[song]}")
                elif song in saved_suffix:
                    contribution_details.append(f"✓ {song_line[2:]} {saved_suffix[song]}")
                else:
                    contribution_details.append(f"✗ {song_line[2:]}")
            ...
            tier_data[tr].append(
                {
                    "Player": p,
                    "GR": {
                        "count": float(round(gen, 2)),
                        "details": [f"{cor}/{tot}"] + player_song_details[p]["Overall"],
                    },
                    "Lives Taken": int(atk),
                    ...
                    "Contribution Rate": {
                        "count": float(round(con, 2)),
                        "details": [f"{int(atk) + int(blk)}/{cor}"] + contribution_details,
                    },
                    "Median Time": t_det,
                    "Chanting GR": {
                        "count": float(round(chn, 2)),
                        "details": [f"{chc}/{cht}"] + player_song_details[p]["Chant"],
                    },
                }
            )
...
def render_dashboard_song(analyzer) -> list[dict]:
    """Formats matrix records for the song distribution breakdown."""
    song_matrix_list = []
    for s in analyzer.song_data:
        if s["vintage"] > 0:
            song_matrix_list.append(
                {
...
def render_dashboard_plot(
    analyzer, avg_rank: float, raw_vintage_by_guess: dict, raw_vintage_by_list: dict
) -> tuple[list[dict], list[dict]]:
    """Calculates coordinates, node metadata, and vector connections for interactive scatter plots."""
    pool_data = []
    ...
    for name in analyzer.s_part:
        if analyzer.c_counts[name] > 0:
            ...
            base_node = {
                ...
                "gr": float(round(analyzer.c_counts[name] / analyzer.s_part[name] * 100, 2))
                if analyzer.s_part[name]
                else 0.0,
                "rig_gr": float(round(analyzer.p_rigs_h[name] / analyzer.p_rigs[name] * 100, 2))
                if analyzer.p_rigs[name]
                else 0.0,
                "performance": float(round(perf_score, 2)),
                "rig_rate": float(round(analyzer.p_rigs[name] / analyzer.s_part[name] * 100, 2))
                if analyzer.s_part[name]
                else 0.0,
            }
            ...
            if analyzer.p_l_corr[name] and pd.notnull(yl) and pd.notnull(yg):
                ...
                hit_vint = (
                    np.median(analyzer.p_lh_vint[name]) if analyzer.p_lh_vint[name] else base_node["vintage"]
                )
...
def create_dashboard_html(analyzer, path: Path, use_teams: bool, watched: bool):
    """Generates site/data.json and site/jsons/search.json and copies dashboard templates."""
    ...
    valid_elos = [
        float(v)
        for v in analyzer.elo_map.values()
        if str(v).replace(".", "", 1).isdigit()
        or (str(v).startswith("-") and str(v)[1:].replace(".", "", 1).isdigit())
    ]
    avg_rank = np.mean(valid_elos) if valid_elos else 1.0
    team_count = len(analyzer.rosters) if analyzer.use_teams else 0

    if team_count <= 2:
        stage = "Final" if analyzer.base_exp >= 3 else f"R{analyzer.base_exp}"
    else:
        stage = (
            "Mid-Tour"
            if analyzer.base_exp == 3
            else "Final"
            if (team_count <= 4 and analyzer.base_exp >= 6) or (team_count > 4 and analyzer.base_exp >= 5)
            else f"R{analyzer.base_exp}"
        )
    ...
    df_base, _ = compute_player_rows(
        analyzer,
        analyzer.elo_map,
        analyzer.apps,
        analyzer.exp_map,
        analyzer.base_exp,
        analyzer.new_players,
        watched,
        active,
        t_labels,
        avg_rank,
    ...
    def player_sort_key(x):
        ...
        try:
            elo = float(analyzer.elo_map.get(x.lower(), float("inf")))
        except Exception:
            elo = float("inf")
        return (gr, -elo)
    ...
    render_teams, render_team_hl_rules = render_dashboard_team(
        analyzer, df_teams, analyzer.team_song_details
    )
    render_scatter, render_arrows = render_dashboard_plot(
        analyzer, avg_rank, analyzer.raw_vintage_by_guess, analyzer.raw_vintage_by_list
    )

    render_tour_stats = render_dashboard_tour(
        analyzer, watched, analyzer.tour_song_details, analyzer.player_song_details
    )
    render_tier_merged = render_dashboard_tier(
        analyzer, rows1, rows2, analyzer.player_song_details
    )
    ...
    for path_json in analyzer.json_paths:
                ...
                for m_p in analyzer.rosters[tid]:
                    if m_p.lower() not in analyzer.assignments:
                        for c_p in raw_f_players:
                            if (
                                c_p.lower() in analyzer.assignments
                                and analyzer.assignments[c_p.lower()][0] == tid
                            ):
                                analyzer.assignments[m_p.lower()] = analyzer.assignments[c_p.lower()]
            if len(final_members) < 8:
                for tid in t_in_f:
                    final_members.update(analyzer.rosters[tid])
        ...
        for song in data_j.get("songs", []):
            ...
            anime_type = (
                "Movie"
                if anime_type_raw.lower() == "movie"
                else "Special"
                if anime_type_raw.lower() == "special"
                else anime_type_raw
            )
            ...
            for p in raw_correct:
                if isinstance(p, dict) and "name" in p:
                    ...
                    guess_times[p.lower()] = "N/A"

            guessers_flat = [
                p if isinstance(p, str) else p["name"] for p in raw_correct if isinstance(p, (str, dict))
            ]
            ...
            if analyzer.use_teams:
                ...
                if len(t_list) == 2:
                    ...
                    cA, cB = correct_set & analyzer.rosters[tA], correct_set & analyzer.rosters[tB]
            ...
            def group_by_team_structure(target_players, include_times=False):
                if not analyzer.use_teams:
                    ...
                    if include_times:
                        return [f"{p} ({guess_times.get(p.lower(), 'N/A')})" for p in sorted_p]
                    return sorted_p
                ...
                for tid in sorted_tids:
                    ...
                    pts_sorted = sorted(
                        team_buckets[tid], key=lambda x: analyzer.assignments.get(x.lower(), (None, "5"))[1]
                    )
                    p_strings = (
                        [f"{p} ({guess_times.get(p.lower(), 'N/A')})" for p in pts_sorted]
                        if include_times
                        else pts_sorted
                    )
                ...
                all_active_tids = {
                    analyzer.assignments[p.lower()][0] for p in final_members if p.lower() in analyzer.assignments
                }
            ...
            raw_genres = (
                si.get("animeGenre", []) if isinstance(si.get("animeGenre"), list) else []
            )
            raw_tags = (
            ...
            raw_alts_ans = (
                si.get("altAnimeNamesAnswers", []) if isinstance(si.get("altAnimeNamesAnswers"), list) else []
            )
            titles_to_check = {anime_romaji.lower().strip(), anime_english.lower().strip()}
            combined_alts = list(
                {
                    alt.strip()
                    for alt in (raw_alts + raw_alts_ans)
                    if isinstance(alt, str) and alt.strip().lower() not in titles_to_check
                }
            )
            ...
            search_songs_list.append(
                {
                    ...
                    "correct_teams_flat": [
                        analyzer.assignments[p.lower()][0]
                        for p in guessers_flat
                        if p.lower() in analyzer.assignments
                    ],
                    "alts": combined_alts,
    ...
    with open(path / "jsons" / "search.json", "w", encoding="utf-8") as f:
        json.dump(search_songs_list, f, ensure_ascii=False, indent=4)
    with open(path / "jsons" / "data.json", "w", encoding="utf-8") as f:

...
def create_player_png(
    ...
    """Generates the main Player Statistics table PNG image."""
    ...
    df, mask = compute_player_rows(
        analyzer, elo_map, apps, exp_map, base_exp, new_players, watched, active, t_labels, avg_rank
    )
    df_png = df.copy()
    pcts = (
        ["GR"]
        + [t_labels[t] for t in active]
        + (["Rig Rate", "Solo Rig Rate", "Rig Δ", "Rig GR", "Off GR"] if watched else [])
        + ["Chant GR"]
    )

    if "Elo" in df_png.columns:
        df_png["Elo"] = pd.to_numeric(df_png["Elo"], errors="coerce").map(
            lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A"
        )
    if "UF" in df_png.columns:
        df_png["UF"] = pd.to_numeric(df_png["UF"], errors="coerce").map(
            lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A"
        )
    if "Score" in df_png.columns:
        df_png["Score"] = pd.to_numeric(df_png["Score"], errors="coerce").map(
            lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A"
        )
    if "Median Time" in df_png.columns:
        df_png["Median Time"] = pd.to_numeric(df_png["Median Time"], errors="coerce").map(
            lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A"
        )
    if "Mean Over-8" in df_png.columns:
        df_png["Mean Over-8"] = pd.to_numeric(df_png["Mean Over-8"], errors="coerce").map(
            lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A"
        )
    if "Rig Over-8" in df_png.columns:
        df_png["Rig Over-8"] = pd.to_numeric(df_png["Rig Over-8"], errors="coerce").map(
            lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A"
        )
    if "Over-8 Δ" in df_png.columns:
        df_png["Over-8 Δ"] = pd.to_numeric(df_png["Over-8 Δ"], errors="coerce").map(
            lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A"
        )

    for c in pcts:
        df_png[c] = (
            pd.to_numeric(df_png[c], errors="coerce")
            .mul(100)
            .map(lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A")
        )

    delta_cols = ["GR Δ", "UF Δ", "OP Δ", "ED Δ", "IN Δ"]
    for dc in delta_cols:
        if dc in df_png.columns:
            df_png[dc] = pd.to_numeric(df_png[dc], errors="coerce").map(
                lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A"
            )
    ...
    export_png(
        ...
        path,
        "Player.png",
        f"{prefix}Player Statistics, {stage}",
        mask,
        val_str,
    )
...
def create_tour_png(analyzer, use_teams: bool, watched: bool, path: Path):
    """Generates the Tour Statistics table PNG image."""
...
def create_team_png(analyzer, assigns: dict, t1_lookup: dict, path: Path):
    """Generates the Team Statistics table PNG image."""
    analyzer.text_var_wlt = "Yes"
    df = compute_team_rows(analyzer, assigns, t1_lookup)

    if "_win_pct" in df.columns:
        df_png = df.drop(columns=["_tid", "_history", "_win_pct"])
    else:
        df_png = df.drop(columns=["_tid", "_history"])

    if "Win Record" in df_png.columns and (
        (df_png["Win Record"].astype(str) == "0-0-0").all()
        or (df_png["Win Record"].astype(str) == "0-0").all()
    ):
        df_png = df_png.drop(columns=["Win Record"])
    ...
    for c in num_cols:
        df_png[c] = pd.to_numeric(df_png[c], errors="coerce").map(
            lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A"
        )
...
def create_tier_png(analyzer, assigns: dict, path: Path, has_chanting_songs: bool):
    """Generates the Tier Statistics table PNG image using HTML rendering."""
    ...
    best_gen_idx = (
        len(rows1) - 1 - max(range(len(rows1)), key=lambda i: rows1[::-1][i]["gen_val"])
        if rows1
        else None
    )
    best_atk_idx = (
        len(rows1) - 1 - max(range(len(rows1)), key=lambda i: rows1[::-1][i]["atk_val"])
        if rows1
        else None
    )
    best_blk_idx = (
        len(rows1) - 1 - max(range(len(rows1)), key=lambda i: rows1[::-1][i]["blk_val"])
        if rows1
        else None
    )
    best_con_idx = (
        len(rows2) - 1 - max(range(len(rows2)), key=lambda i: rows2[::-1][i]["con_val"])
        if rows2
        else None
    )
    ...
    html_parts = [
        "<tr><th>Tier</th><th>Guess Rate</th><th>Lives Taken</th><th>Lives Saved</th></tr>"
    ]
    ...
    full = f"""<html>
        <head>
            <style>
                ...
                th {{ font-weight: bold; font-size: 25px; text-align: center; padding: 10px; border: 1px solid black; border-bottom: 3px solid black; background-color: #f0f0f0; }}
                td {{ font-size: 25px; text-align: center; padding: 10px; border: 1px solid black; }}
    ...
    try:
        trim_whitespace(path / "Tier.png")
    except Exception:
        pass


def create_scatter_png(analyzer, path: Path, list_mode: bool = False, elo_map: dict = None):
    """Generates the Guess Statistics or List Statistics scatter plot PNG graphs."""
    configs = []
    ...
    if list_mode:
        ...
        if plist_l:
            ...
            y_vals_l = [
                np.median(analyzer.p_l_vint[name]) if analyzer.p_l_vint[name] else np.nan
                for name in plist_l
            ]
            ...
            if valid_l:
                ...
                rig_rates = [
                    analyzer.p_rigs[name] / analyzer.s_part[name] if analyzer.s_part[name] else 0
                    for name in plist_l
                ]
                grid_grs = [
                    analyzer.p_rigs_h[name] / analyzer.p_rigs[name] if analyzer.p_rigs[name] else 0
                    for name in plist_l
                ]
                ...
                cmap_l = mc.LinearSegmentedColormap.from_list(
                    "rig_gr_cmap",
                    [(0.0, COLOR_0), (0.7, COLOR_0), (0.8, COLOR_1), (0.9, COLOR_2), (1.0, COLOR_2)],
                )
    ...
    if plist_g:
        ...
        y_vals_g = [
            np.median(analyzer.p_c_vint[name]) if analyzer.p_c_vint[name] else np.nan for name in plist_g
        ]
        ...
        if valid_g:
            ...
            gr_vals = [
                analyzer.c_counts[name] / analyzer.s_part[name] if analyzer.s_part[name] else 0
                for name in plist_g
            ]
            if elo_map is None:
                elo_map = {}
            ...
            if uf_pool and el_pool:
                ...
                if len(els) > 1 and np.var(els) > 0:
                    ...
                    expected_ufs = np.array([np.mean(ufs)] * len(ufs))
            ...
            cmap_g = mc.LinearSegmentedColormap.from_list(
                "guess_uf_elo_cmap", [(0, COLOR_0), (0.5, COLOR_1), (1, COLOR_2)]
            )
    ...
    for cfg in configs:
        ...
        try:
            trim_whitespace(path / cfg["filename"])
        except Exception:
            pass


def create_song_png(analyzer, path: Path):
    """Generates the Song Statistics heat grid PNG image."""
    diffs = [s["difficulty"] for s in analyzer.song_data]
    max_diff = max(diffs) if diffs else 0

    if max_diff < 40:
        num_x, num_y, font_sz = 4, 4, 90
    else:
        num_x, num_y, font_sz = 5, 5, 75
    ...
    for s in analyzer.song_data:
        ...
        if num_y == 4:
            y_idx = 0 if vint < 2000 else min(int(math.floor((vint - 2000) / 10)) + 1, 3)
        else:
    ...
    fig, ax = plt.subplots(figsize=(10, 10))
    cmap_song = mc.LinearSegmentedColormap.from_list(
        "song_cmap",
        [(0.000, COLOR_0), (0.375, COLOR_1), (0.625, COLOR_2), (1.000, COLOR_2)],
    )
    ...
    try:
        trim_whitespace(path / "Song.png")
    except Exception:
        pass


def export_png(analyzer, df: pd.DataFrame, path: Path, fname: str, title: str, mask: list = None, val_str: str = "default"):
    """Generic HTML table screenshot builder used by player, tour, and team renderers."""
    if not analyzer.browser_path:
        return
    ...
    cols_to_drop = [
        c
        for c in delta_check_cols
        if c in df.columns and (df[c].isna() | (df[c].astype(str).str.strip() == "N/A")).all()
    ]
    if cols_to_drop:
    ...
    desc = [
        ...
        "Mean Elo", "Mean GR", "Total 1/8s", "Rig Synergy", "Off Synergy", "Shared Rigs", "Win Record"
    ]
    ...
    elo_ser = (
        pd.to_numeric(df[elo_col], errors="coerce").fillna(0.0)
        if elo_col
    ...
    for col in df.columns:
        if col in desc or col in asc:
            ...
            if not num.dropna().empty:
                ...
                rig_ser = (
                    pd.to_numeric(df["Rigs"], errors="coerce").fillna(0)
                ...
                if col == "Solo Rigs":
                    ...
                    best_idx = (
                        pd.to_numeric(df["GR"], errors="coerce")
                        .fillna(0)
                        .loc[best_b_indices]
                        .idxmin()
                        if not best_b_indices.empty
                        else None
                    )
                    worst_idx = (
                        pd.to_numeric(df["GR"], errors="coerce")
        ...
        for t in th:
            ...
            for i, v in enumerate(gv):
                if pd.notnull(v) and v >= t:
                    f_idx = i
            if f_idx != -1 and f_idx < len(df) - 1:
                borders.append(f_idx)

    col_borders = {
        "Player", "Score", "Mean Over-8", "Lives Saved", "IN Δ", "Rig Rate",
        "Solo Rig Rate", "Over-8 Δ", "Rig Δ", "Median Vintage Hit", "Metric", "Value", "Team Leader"
    }

    if "Score" not in df.columns:
        col_borders.add("GR Δ") if "GR Δ" in df.columns else col_borders.add("GR")
    if "IN Δ" not in df.columns:
        col_borders.add("IN GR")
    ...
    for idx, row in df.iterrows():
        ...
        for cname, cell in row.items():
            ...
            if cname in col_borders:
                style.append("border-right: 3px solid black;")

            if cname == "Mean Difficulty Hit" and pd.notnull(cell) and isinstance(cell, (int, float)):
                cell_display = f"{float(cell):.2f}"
            elif cname == "Median Vintage Hit" and pd.notnull(cell) and isinstance(cell, (int, float)):
                cell_display = format_year(float(cell))
            else:
                cell_display = cell
            ...
            if cname in stats:
                ...
                if cname in desc:
                    if is_max:
                        style.append(f"background-color: {COLOR_2}; color: white; font-weight: bold;")
                    elif is_min and elig:
                        style.append(f"background-color: {COLOR_0}; color: white; font-weight: bold;")
                elif cname in asc:
                    if is_max and elig:
                        style.append(f"background-color: {COLOR_0}; color: white; font-weight: bold;")
                    elif is_min:
    ...
    try:
        trim_whitespace(path / fname)
    except Exception:
        pass


def fuse_images(path: Path):
    """Combines individual generated PNG cards into concatenated General.png, Extra.png, and Plots.png summaries."""
    f = {
        "Tour": "Tour.png",
        "Team": "Team.png",
        "Tier": "Tier.png",
    ...
    if img_team:
        left_components.append(img_team)
    if img_tier:
        left_components.append(img_tier)

...
from hako.help.config import FILE_ALIAS, SCALE_PERF, THRESH_TIME, format_year
...
def compute_player_rows(
    ...
    """Calculates comprehensive individual player metrics, historical deltas, and scores."""
    rows, eligibility = [], []
    ...
    for name in analyzer.s_part:
                ...
                with open(alias_txt_path, "r", encoding="utf-8") as f_alias:
                    for a_line in f_alias:
                        if "," in a_line:
                            ...
                            if len(p_splits) >= 7 and (
                                p_splits[0].lower() == name.lower() or p_splits[1].lower() == name.lower()
                            ):
                                if p_splits[2] != "N/A":
                                    history_baselines = {
                                        "GR": float(p_splits[2]),
        ...
        if analyzer.use_teams:
            row["Elo"] = elo_map.get(name.lower(), np.nan)
        ...
        if analyzer.use_teams:
            ...
            try:
                elo = float(elo_map.get(name.lower(), 0.0))
            except Exception:
                elo = 0.0

            all_ufs = [
                (analyzer.p_usefulness_sum[p] * avg_rank * 8) / analyzer.s_part[p] if analyzer.s_part[p] else 0.0
            ...
            for p in analyzer.s_part:
                try:
                    all_elos.append(float(elo_map.get(p.lower(), 0.0)))
                except Exception:
                    all_elos.append(0.0)
            ...
            if len(all_elos) > 1 and np.var(all_elos) > 0:
                ...
                slope, intercept, res_std = 0, np.mean(all_ufs) if all_ufs else 0, 1
        ...
        row.update(
            {
                "1/8s": analyzer.e_counts[name],
                "2/8s": analyzer.p_two_e[name],
                "7/8s": analyzer.p_rev_e[name],
                "Mean Over-8": avg_over8,
            }
        )
        if analyzer.use_teams:
            row.update({"Lives Taken": analyzer.p_pts[name], "Lives Saved": analyzer.p_blks[name]})
            ...
            row.update(
                {
                    ...
                    "Solo Rig Rate": analyzer.p_l_solos[name] / analyzer.p_rigs[name]
                    ...
                    "Off GR": (cor - analyzer.p_rigs_h[name]) / (tot - analyzer.p_rigs[name])
                    if (tot - analyzer.p_rigs[name])
                    else np.nan,
    ...
    if "Score" in df.columns:
        df = df.sort_values(by=["GR", "Score"], ascending=[False, False])
        ...
        df = df.sort_values("GR", ascending=False)
...
def compute_tour_stats(analyzer, use_teams: bool, watched: bool) -> list:
    """Computes high-level aggregated tournament statistics and top performer records."""
    from hako.help.config import CONST_CONV
    ...
    stats = [
        ...
        [
            "Mean GR",
            f"{100 * (analyzer.global_stats['tot_c'] / sum(analyzer.s_part.values())):.2f}" if analyzer.s_part else "N/A",
            None,
        ],
    ...
    if use_teams:
        stats.append(["Total 4-0s", analyzer.global_stats["sweeps"], "Total 4-0s"])
    ...
    no_s = sorted(
        [n for n in plist if analyzer.e_counts[n] == 0 and analyzer.s_part[n] > 0],
        key=lambda x: analyzer.c_counts[x] / analyzer.s_part[x],
        reverse=True,
    )
    yes_s = sorted(
        [n for n in plist if analyzer.e_counts[n] > 0 and analyzer.s_part[n] > 0],
    ...
    if no_s:
        stats.append(
            [
                "Highest GR Without 1/8s",
                f"{no_s[0]} ({100 * (analyzer.c_counts[no_s[0]] / analyzer.s_part[no_s[0]]):.2f})",
                None,
            ]
        )
    if yes_s:
        stats.append(
            [
                "Lowest GR With 1/8s",
                f"{yes_s[0]} ({100 * (analyzer.c_counts[yes_s[0]] / analyzer.s_part[yes_s[0]]):.2f}, {analyzer.e_counts[yes_s[0]]})",
                ("1/8s", yes_s[0]),
            ]
        )
    ...
    if watched:
        ...
        if eligible:
            ...
            stats.append(
                ["Worst Solo Rig Converter", f"{w['n']} ({w['p']:.2f}, {w['h']}/{w['t']})", ("Solo Rigs", w["n"])]
            )
...
def compute_team_rows(analyzer, assigns: dict, t1_lookup: dict) -> pd.DataFrame:
    """Calculates team-level performance, list synergy, and match histories."""
    from collections import defaultdict
    import json
    import re
    from hako.help.config import FILE_ALIAS, FILE_CODES
    from .integrations import download_challonge_page, parse_challonge_display_leader

    if not getattr(analyzer, "use_teams", False):
        return pd.DataFrame()
            ...
            try:
                ...
                for script in soup.find_all("script"):
                    if script.string and "window._initialStoreState" in script.string:
                        match = re.search(
                            r"window\._initialStoreState\['TournamentStore'\]\s*=\s*({.*?});", script.string, re.DOTALL
                        )
                        ...
                        if match:
                            ...
                            for _, rounds in s_data.get("matches_by_round", {}).items():
                                chal_matches.extend(rounds)
                            break
            except Exception as e:
                print(f"[!] Failed to fetch Challonge data: {e}")
    ...
    for tid in analyzer.t_c_ps:
        ...
        if leader_name.lower() in alias_map:
            leader_variants.add(alias_map[leader_name.lower()])
        ...
        for m in chal_matches:
            ...
            if p1_match or p2_match:
                if p1_match:
                    my_score, opp_score, opp_name = s1, s2, p2_leader
                else:
                    my_score, opp_score, opp_name = s2, s1, p1_leader
...
def compute_tier_rows(analyzer, assigns: dict, has_chanting_songs: bool) -> tuple[list[dict], list[dict]]:
    """Calculates performance distributions and top performers grouped by team tier (1 to 4)."""
    rows1, rows2 = [], []
    ...
    for tr in ["1", "2", "3", "4"]:
        ...
        for p in tp:
            ...
            if has_chanting_songs:
                chn_players.append({"player": p, "value": chn})
        ...
        row2["Contribution Rate"] = (
            f"{con_players[0]['player']} ({con_players[0]['value']:.2f})" if con_players else "N/A"
        )
        row2["Median Time"] = f"{spd_players[0]['player']} ({spd_players[0]['value']:.2f})" if spd_players else "N/A"
        row2["Chant GR"] = (
            f"{chn_players[0]['player']} ({chn_players[0]['value']:.2f})" if chn_players and chn_players[0]["value"] > 0 else ""
        )

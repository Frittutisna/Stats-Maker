...
function initQuizSettingsCheckboxes() {
    ...
    masterChk.addEventListener("change", () => {
        ...
        quizConfig.forEach(c => {
            ...
            else if (!isChecked && c.type === "range") c.currentMin = c.min; c.currentMax = c.max;
        });
    ...
    quizConfig.forEach(col => {
        ...
        chk.addEventListener("change", () => {
            ...
            if (col.type === "categorical" && col.subOptions) {
                ...
                col.currentMin = col.min; col.currentMax = col.max;
                const inputs = colWrapper.querySelectorAll("input[type='number']");
...
const nativeSwitchDashboardTab = window.switchDashboardTab;

window.switchDashboardTab = function(evt, tabId) {
    if (tabId !== 'quiz-tab')       exitQuizEngine();
    if (nativeSwitchDashboardTab)   nativeSwitchDashboardTab(evt, tabId);

    const gearWrapper = document.getElementById('globalGearWrapper');
    const helpWrapper = document.getElementById('globalHelpWrapper');

    if (tabId === 'quiz-tab') {
        if (gearWrapper) gearWrapper.classList.remove('invisible');
        if (helpWrapper) helpWrapper.classList.remove('invisible');
    }
...
const nativeToggleGlobalGear = window.toggleGlobalGear;
...
window.toggleGlobalGear = function(event) {
    ...
    if (activeTab === 'quiz-tab') {
        ...
        if (nativeToggleGlobalGear) nativeToggleGlobalGear(event);
    }
};

const nativeToggleGlobalHelp = window.toggleGlobalHelp;
...
window.toggleGlobalHelp = function(event) {
    ...
    if (activeTab === 'quiz-tab') {
        ...
        if (nativeToggleGlobalHelp) nativeToggleGlobalHelp(event);
    }
};

document.getElementById("quizSettingsDropdown")?.addEventListener("click", (e) => e.stopPropagation());

const checkIntervalForData = setInterval(() => {
    if (globalSearchData && globalSearchData.length > 0 && players && players.length > 0) {
...
function toggleQuizPause() {
    ...
    document.getElementById("globalQuizPauseBtn").innerText = quizIsPaused ? "▶" : "❚❚";

    if (quizIsPaused && quizLeewayTimeoutId) {clearTimeout(quizLeewayTimeoutId); quizLeewayTimeoutId = null;}
...
function finishQuizEngine() {
    ...
    document.getElementById("quizResolutionLabel")  .innerText = `Score: ${quizScore}/${quizActivePool.length}`;
    document.getElementById("quizResolutionLabel")  .removeAttribute("title");
    document.getElementById("quizTimerValue")       .innerText = "0";
    document.getElementById("globalQuizPauseBtn")   .classList.add("hidden");
}
...
function exitQuizEngine() {
    ...
    document.getElementById("quizAudioAnchor")      .innerHTML = "";
    document.getElementById("quizActiveScreen")     .classList.add      ("hidden");
    document.getElementById("quizSetupScreen")      .classList.remove   ("hidden");
    document.getElementById("globalQuizPauseBtn")   .classList.add      ("hidden");
    document.getElementById("globalQuizReturnBtn")  .classList.add      ("hidden");
}

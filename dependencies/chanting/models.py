...
class SongCategory(Enum):
    ...
    def name(self):
        if self == SongCategory.UNKNOWN         : return "Unknown"
        if self == SongCategory.INSTRUMENTAL    : return "Instrumental"
        if self == SongCategory.CHANTING        : return "Chanting"
        if self == SongCategory.CHARACTER       : return "Character"
        if self == SongCategory.STANDARD        : return "Standard"
...
class Type(Enum):
    ...
    def name(self):
        if self == Type.OPENING : return "OP"
        if self == Type.ENDING  : return "ED"
        if self == Type.INSERT  : return "IN"
...
class IntList(UserList):
    def __init__(self, integers         = []): self.data = [int(integer)                for integer         in integers]

class StrList(UserList):
    def __init__(self, strings          = []): self.data = [str(string)                 for string          in strings]

class PathList(UserList):
    def __init__(self, paths            = []): self.data = [Path(path)                  for path            in paths]

class NameList(UserList):
    def __init__(self, names            = []): self.data = [Name(name)                  for name            in names]

    def name(self, language):
        for name in self.data:
            if name.language in language: return name.name
        return self.data[0].name

class TypeList(UserList):
    def __init__(self, types            = []): self.data = [Type(type)                  for type            in types]

class SongCategoryList(UserList):
    def __init__(self, song_categories  = []): self.data = [SongCategory(song_category) for song_category   in song_categories]
...
class Artist(Base):
    ...
    @property
    def id(self)                                        : return self.song_artist_id

    @classmethod
    def from_master_list(cls, master_list, artist_id)   : return cls(master_list['artistMap'][str(artist_id)])
    def all_artists(self)                               : yield self
    def contributing_artists(self)                      : return
...
class Group(Base):
    ...
    @property
    def id(self): return self.song_group_id
    ...
    def all_artists(self):
        yield self
        for artist in self.artists: yield artist
...
class Song(Base):
    ...
    def from_master_list(cls, master_list, song_id):
        ...
        if      master_song['songArtistId']     : song.artist   = Artist    .from_master_list(master_list, master_song['songArtistId'])
        elif    master_song['songGroupId']      : song.artist   = Group     .from_master_list(master_list, master_song['songGroupId'])

        if      master_song['composerArtistId'] : song.composer = Artist    .from_master_list(master_list, master_song['composerArtistId'])
        elif    master_song['composerGroupId']  : song.composer = Group     .from_master_list(master_list, master_song['composerGroupId'])

        if      master_song['arrangerArtistId'] : song.arranger = Artist    .from_master_list(master_list, master_song['arrangerArtistId'])
        elif    master_song['arrangerGroupId']  : song.arranger = Group     .from_master_list(master_list, master_song['arrangerGroupId'])

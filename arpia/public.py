...
def main():
    print("Which tours should be processed?")
    while True:
        tour_id = input("Select tour [0/1/2]: ").strip()
        if tour_id in ["0", "1", "2"]:
            break
        print("Please input a valid choice")
        
    TOUR_DIR = os.path.join(ROOT_DIR, "tours", tour_id)
    JSONS = os.path.join(TOUR_DIR, "jsons")
    TEAMS = os.path.join(TOUR_DIR, "codes.txt")
    ARPIA_DIR = os.path.join(TOUR_DIR, "arpia")
    os.makedirs(ARPIA_DIR, exist_ok=True)
...
"""
    ...
    gc = get_gspread_client(ROOT_DIR)
    sync_chanting_ids_file(gc, ROOT_DIR)
    ...
    for file_name in os.listdir(JSONS):
        ...
        for song in json_data['songs'][:songs_played]:
            ...
            if is_list:
                lobby_guesses = len(song.get("correctGuessPlayers", []))
                for list_state in song.get("listStates", []):
    ...
    saveSongStats(songDB=songDB, path=ARPIA_DIR, filename="Songs.png")
    ...
    separators = ["Player name", "Usefulness", "7/8s", "# 3/8s or below", "Lives saved", "Avg vintage played", "Total songs"]
    if single_song_type:
    ...
    path = os.path.join(ARPIA_DIR, "Player.png")
    df_to_png(df=final_df1, path=ARPIA_DIR, filename="Player.png", reverse_cols=reverse_columns, exclude_columns=exclude_columns, separators=separators)
    print(f"Stats about GR saved at {path}")

    exclude_columns = ["Rank", "Guess rate"]
    separators = [separator for separator in ["Player name", "\u0394UF", "# OPs played", "# EDs played", "# INs played"] if separator in finalOrder2]

    path2 = os.path.join(ARPIA_DIR, "Types.png")
    df_to_png(df=final_df2, path=ARPIA_DIR, filename="Types.png", reverse_cols=reverse_columns, exclude_columns=exclude_columns, separators=separators)
    ...
    if is_list:
        ...
        path3 = os.path.join(ARPIA_DIR, "List.png")
        df_to_png(df=final_df3, path=ARPIA_DIR, filename="List.png", reverse_cols=reverse_columns, exclude_columns=exclude_columns, separators=separators)
        print(f"Stats about watched saved at {path3}")

    export_extra_stats_screenshot(server_average_mode, root_dir=ROOT_DIR, tour_id=tour_id, gc=gc)
...
def append_log_rows(worksheet, rows):
    ...
    try:
        target_rows = max(getattr(worksheet, "row_count", 0), start_row + len(padded_rows) - 1)
        target_cols = max(getattr(worksheet, "col_count", 0), max_cols)
...
def get_incorrect_guess_players(song, game_player_names, alias_to_id):
    correct_players = song.get("correctGuessPlayers", [])
    if not isinstance(correct_players, list):
        correct_players = []
...
def ordered_song_json_values(song, game_player_names, alias_to_id):
    ...
    for field in JSON_DATA_FIELDS:
        if field == "correctCount":
            correct_players = song.get("correctGuessPlayers", [])
            values.append(len(correct_players) if isinstance(correct_players, list) else "")
...
def log_export_data(sheet, tour_type, export_time, list_guess_counts, raw_json_songs, alias_to_id, id_to_aliases):
    ...
    if json_rows:
        ...
        rows_to_append = json_rows + [[""]]
        if existing_json_rows:
            rows_to_append = [[""]] + rows_to_append
        append_log_rows(json_wks, rows_to_append)
...
def get_gspread_client(root_dir):
    credentials_filename = os.path.join(root_dir, "dependencies", "credentials", "credentials.json")
    authorized_user_filename = os.path.join(root_dir, "dependencies", "credentials", "authorized_user.json")
...
def sync_chanting_ids_file(gc, root_dir):
    chanting_ids = load_chanting_ids(gc)
    deps_dir = os.path.join(root_dir, "dependencies")
    os.makedirs(deps_dir, exist_ok=True)
    with open(os.path.join(deps_dir, "chanting.txt"), "w", encoding="utf-8") as f:
...
def export_extra_stats_screenshot(server_average_mode, root_dir, tour_id, gc=None, ask_cleanup=False):
    tour_dir = os.path.join(root_dir, "tours", tour_id)
    arpia_dir = os.path.join(tour_dir, "arpia")
    json_dir = os.path.join(tour_dir, "jsons")
    ...
    codes_path = os.path.join(tour_dir, "codes.txt")
    codes_valid = False
        ...
        if all_teams_data:
            ...
            for t_idx, members in enumerate(all_teams_data, 1):
                for i, p_in in enumerate(members):
                    tier = f"T1" if i == 0 else f"T2" if i == 1 else f"T3" if i == 2 else f"T4"
    ...
    try:
        save_extra_stats_image(extra_image_data, arpia_dir, image_name)
    except Exception as exc:
    ...
    return os.path.join(arpia_dir, image_name)
